...
int latticePoints(pt a,pt b) {
    return gcd((int)(abs(a.x - b.x)), 
        (int)(abs(a.y - b.y))) - 1; }
// rotate pt p by angle 'a': center origin, counterclockwise
pt rot(pt p, ld a) {
    return {T(p.x*cos(a) - p.y*sin(a)), 
            T(p.x*sin(a) + p.y*cos(a))};}
...
pt rot(pt c, pt b, ld a) 
    {return c + rot(b - c, a);}
pt linearTransformation(pt p,pt q,pt r,
    pt fp,pt fq) { return 
    fp + (r - p) * ((fq - fp).x/ (q-p).x);}
ld angle(pt a,pt b) {
...
//if segment [a,b],[c,d] intersect properly(~endpoints)
...
struct cmp { // make_pr => make_pair
 bool operator()(pt a, pt b) {
 return make_pr(a.x,a.y)<make_pr(b.x,b.y);}};
...
ld segSegDist(pt a,pt b,pt c,pt d) {
 ...
 return min({segPointDist(a, b, c),
  segPointDist(a, b, d),segPointDist(c, d, a),
  segPointDist(c, d, b) }); }
//counterclockwise order,ifin clockwise order
//area is negativefor both concave and convex
...
bool inPolygon(vector<pt>&v,pt a,
      bool strict=true){int numCrossings = 0;
...
// returns how many times we go anticlockwise around a, following polyline, (a should not lie on polyline) if point is outside polygon, winding number is zero
...
make_tuple(b.t, half(b.d), cross(a.d, b.d));}
 ang t180() {return{d * (-1), t + half(d)};}
    ...
    ang b{newD, a.t}; if (a.t180() < b) b.t--;
    if (b.t180() < a) b.t++;   return b;}};
...
pt circumCenter(pt a,pt b,pt c) {
    b = b - a, c = c - a;
    assert(cross(b, c) != 0);
    return a + prep(b * sq(c) - c * sq(b)) 
                / cross(b, c) / 2;}
int circleLine(pt o,ld r,line l,
        pair<pt,pt>&out) {
    ld h2 = r * r - l.sqDist(o);
    if (h2 >= 0) { pt p = l.proj(o);
        pt h = l.v * sqrt(h2) / abs(l.v);
     out = {p-h,p+h};} return 1 + sgn(h2);}
int circleCircle(pt o1,ld r1,pt o2,ld r2,
            pair<pt,pt>&out) {
    pt d = o2 - o1; ld d2 = sq(d);
    if (d2 == 0) {
assert(r1 != r2); return 0;/*cncentrc crcl*/}
...
for(int i=1;i<a.size();i++)
 if(a[i].x<a[pos].x || 
 (a[i].x==a[pos].x && a[i].y<a[pos].y))pos=i;
...
vector<pt> minkowskiSum(vector<pt>a,vector<pt>b) {
    reorder(a); reorder(b); vector<pt> ans;
 int i = 0, j = 0, n = sz(a), m = sz(b),times=n+m;
while (times--) { ans.push_back(a[i] + b[j]);
if (cross(a[(i+1)%n]-a[i],b[(j+1)%m]-b[j]) >= 0){
    i++; i %= n; } else { j++; j %= m; } }
    // remove redundant points
    vector<pt> r_ans;
    for (int l = 0; l < n + m; l++) {
        if (orient(ans[(l - 1 + n + m) % (n + m)],
             ans[l], ans[(l + 1) % (n + m)]) != 0) 
        { r_ans.push_back(ans[l]); } }
    return r_ans; }
// lattice points in a polygon strictly inside, s=i+b/2-1
pair<int,int> latticePoints(vector<pt>&p) {
    int area = areaPolygonTw(p), bdryPoints = 0;
    int n = p.size();
...
bool ok(pt &a,pt &b,pt &c,
 bool includeCollinear){int o=orient(a,b,c);
 return o>0 || (includeCollinear && o== 0);}
vector<pt> convexHullGC(vector<pt>a,
...
stk.push_back(a[1]);
for (int i = 2; i < n; i++) {
 while (stk.size()>= 2 && !ok(stk[sz(stk)-2],
  stk[sz(stk)-1], a[i], includeCollinear)) {
...
int diameter(vector<pt>a) {
    ...
    while (cross(a[(ptr_a + 1) % n] - a[ptr_a], a[(ptr_b + 1) % n] - a[ptr_b]) > 0) {
        ptr_b++; ptr_b %= n; }
int ans=dot(a[ptr_b]-a[ptr_a],
            a[ptr_b]-a[ptr_a]);
    int begin_a = ptr_a,begin_b = ptr_b;
  do { if (cross(a[(ptr_a+1) % n] - a[ptr_a],
    a[(ptr_b + 1) % n] - a[ptr_b]) > 0) {
       ptr_b++, ptr_b %= n; } else {
            ptr_a++,ptr_a %= n; }
