// IF DAG : calculate d with dp to reduce time complexity to O(n+m) in init_potential; if unit capacity and unit cost -> use small dijkstra -> O( (n + m) * flow ) Works for both directed, undirected and with negative cost too doesn't work for negative cycles for undirected edges just make the directed flag false Complexity: O(min(E^2 *V log V, E logV * flow))
using T = long long;
const T inf = 1LL << 61;
struct MCMF {
	struct edge {
	int u, v, id; T cap, cost;
		edge(int _u, int _v, T _cap, T _cost, int _id) : u(_u), v(_v), cap(_cap), cost(_cost), id(_id) {} };
	int n, s, t, mxid;
	T flow, cost;
	vector<vector<int>> g; vector<edge> e;
	vector<T> d, potential, flow_through; vector<int> par;
	bool neg; MCMF() {} // 0-based indexing
	MCMF(int _n) { n = _n + 10; neg = false; mxid = 0; g.assign(n, vector<int> ());}
	void add_edge(int u, int v, T cap, T cost, int id = -1, bool directed = true) {
		if (cost < 0) neg = true;
		g[u].push_back(e.size()); e.push_back(edge(u, v, cap, cost, id));
		g[v].push_back(e.size()); e.push_back(edge(v, u, 0, -cost, -1));
		mxid = max(mxid, id); if (!directed) add_edge(v, u, cap, cost, -1, true); }
	bool dijkstra() {
		par.assign(n, -1);
		d.assign(n, inf);
		...
		while (!q.empty()) {
			int u = q.top().second; T nw = q.top().first; q.pop();
			if (nw != d[u]) continue;
	...
	T send_flow(int v, T cur) {
		if (par[v] == -1) return cur; int id = par[v];
		T w = e[id].cost, f = send_flow(e[id].u, min(cur, e[id].cap));
		cost += f * w; e[id].cap -= f; e[id ^ 1].cap += f; return f; }
	void init_potential() {
	...
	pair<T, T> solve(int _s, int _t, T goal = inf) {
		...
		potential.assign(n, 0); if (neg) init_potential();
