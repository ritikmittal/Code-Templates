...
struct MCMF {
 ...
 T send_flow(int v, T cur) {
	 if(par[v]==-1)return cur; int id=par[v];
	  T w = e[id].cost;
   T f=send_flow(e[id].u,min(cur,e[id].cap));
   cost += f*w;   e[id].cap-=f;
