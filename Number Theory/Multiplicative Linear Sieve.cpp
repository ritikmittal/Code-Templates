...
// if there is some other random multiplicative function, find f[p] , f[i*p]=f[i]*f[p](if p does not divide i) , f[i*p]=f[i/ p**cnt[i]]*f[p**(cnt[i]+1](if p divides i)
void sieve (int n) {
...
if (!is_composite[i]) {
 ...
 ++j){ is_composite[i * prime[j]] = true;
spf[i*prime[j]]=prime[j];if(i%prime[j]==0){
 phi[i * prime[j]] = phi[i] * prime[j];	//prime[j] divides i
 mu[i * prime[j]] = 0;
 cnt[i * prime[j]] = cnt[i] + 1; break;
 } else { phi[i * prime[j]] = phi[i] * phi[prime[j]];	//prime[j] does not divide i
